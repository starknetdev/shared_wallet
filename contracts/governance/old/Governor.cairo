...
@event
func ProposalCancelled(proposal_id : felt):
end
...
func proposal_snapshot{
        ...
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }(proposal_id : felt) -> (vote_start : felt):
    let (vote_start) = _proposals.read(
        index=proposal_id,
...
@view
func proposal_deadline{
        syscall_ptr : felt*,
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }(proposal_id : felt) -> (vote_end : felt):
    let (vote_end) = _proposals.read(
        index=proposal_id,
        field=ProposalCore.vote_end
    )
    return (vote_end)
end
...
func cast_vote_by_sig{
    ...
    let (weight) = _cast_vote(proposal_id, voter, support, "")
    return (weight)
end


func cast_vote_with_reason_and_params_by_sig{
        syscall_ptr : felt*,
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }(
    ...
    let (weight) = _cast_vote(proposal_id, voter, support, reason, params)
    return (weight)
end
...
func _cast_vote{
        ...
        weight : felt)
    :
    let (weight) = _cast_vote(proposal_id, account, support, reason, _default_params())
    return (weight)
end
...
func _cast_vote{
    ...
    let (state) = proposal_state(proposal_id)
    with_attr error_message("Governor: vote not currently active"):
        assert state = ProposalState.active
    end
    let (weight) = get_votes(account, proposal.vote_start, params)
    _countVote(proposal_id, account, support, weight, params)

    if params.length == 0:
        emit VoteCast(account, proposal_id, support, weight, reason)
    else:
        emit VoteCastWithParams(account, proposal_id, support, weight, reason, params)
    end
